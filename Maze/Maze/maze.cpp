...
void renderScene(void) {
	...
	if (viewState == 0){
		...
		GLfloat lightPosition1[] = {-30.0f,(GLfloat)15*levels+20,20.0f, 0.0f };
...
void processNormalKeys(unsigned char key, int x, int y) {
	...
	else if ((key == 'A' || key == 'a' )&& viewState==1){// When user press x the game ends and the score becoms 0
		deltaAnglex = -0.05f;
	...
	else if ((key == 'D' || key == 'd' )&& viewState==1){// When user press x the game ends and the score becoms 0
		deltaAnglex = 0.05f;
		// update camera's direction
		lx = sin(anglex + deltaAnglex);
		lz = -cos(anglex + deltaAnglex);
...
void key_spe(int k, int x, int y){

	if ((k == GLUT_KEY_UP)&& viewState==1){// When user press x the game ends and the score becoms 0
		deltaAngley = -0.05f;
		ly = -sin(angley + deltaAngley);
		angley += deltaAngley;
	}

	else if((k == GLUT_KEY_DOWN)&& viewState==1){
		deltaAngley = 0.05f;
		ly = -sin(angley + deltaAngley);
		angley += deltaAngley;
	}
